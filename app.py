...
def task():
    ...
    # pprint(change_in_stock_table)
    return {"current_stock_table":current_stock_table, "new_stock_table":new_stock_table, "change_in_stock_table":change_in_stock_table}
...
@app.post("/new_stock")
async def create_stock(newstock: NewStock):
    push_new_stock(item_names_qty=str(newstock.item_names_qty), a_s_invest=newstock.a_s_invest, time=newstock.time)
    result=task()
    conn.commit()
    return result
...
@app.put("/change_in_stock")
async def update_change_in_stock(changeinstock: ChangeInStock):
    record_change_in_stock(item_names_qty=str(changeinstock.item_names_qty), person_updated=changeinstock.person_updated, event=changeinstock.event, time=changeinstock.time)
    result=task()
    conn.commit()
    return result
